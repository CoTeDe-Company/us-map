...
(function($, document, window, Raphael, undefined) {
  ...
  var paths= {
    ...
    PA: {
      id:"pa",
      d: "M1238.9,247.912c-1.928-7.1-17.608-34.2-14.614-38.716s14.005-39.376,18.521-40.644c.812-.254,1.573-1.523,2.334-3.552,0,0,4.872,2.283,8.626,1.776,0,0,1.167,25.472-3.044,32.576-4.212,7.155-2.081,15.527-2.943,20.145-.761,4.414-8.372,26.944-8.829,28.364Z",
      name: "PALESTINE",
      capital:"Jerusalem",
      area:"27,027",
      showName: false
    ...
    LE: {
      id:"le",
      d: "M1245.709,163.243c3.933-9.914,7.7-35.968,12.962-38.243,0,0,11.412.271,13.738,2.925,2.272,2.708,7.756,10.292,0,14.3-7.81,4.063-10.58,16.576-13.738,21.018a4.732,4.732,0,0,1-3.545,1.9C1251.027,165.68,1245.709,163.243,1245.709,163.243Z",
      name: "LEBANON",
      capital:"Beirut",
      area:"10,452",
      showName: false
    ...
    QA: {
      id:"qa",
      d: "M1587.2,342.081l4.163-2.081h3.122s6.244-17.691,2.081-27.057-8.325-6.244-8.325-6.244L1582,316.065v21.854Z",
      name: "QATAR",
      capital:"Doha",
      area:"11,586",
      showName: false 
    ...
    DJ: {
      id:"dj",
      d: "M1393,650h18c2.3-2.521,3.664-13.916,9-8,5.722,6.345-3-7-7-7s2-5,2-5h5s6,1-1-9-19-6-19-6S1388,610,1393,650Z",
      name: "DJIBOUTI",
      capital:"DJIBOUTI",
      area:"23,200",
      showName: false
    ...
    KU: {
      id:"ku",
      d: "M1512,266s-11-9-9-12,22-15,24-14,10,2,10,2,4,7,2,9-2,5-5,7a20.431,20.431,0,0,0-5,5s-3,1,2,3,6,3,6,3l2,4s3,0,3,1,1,4-1,4-10,3-16,1S1512,266,1512,266Z",
      name: "KUWAIT",
      capital:"KUWAIT",
      area:"17,818",
      showName: false
    ...
    BA: {
      id:"ba",
      d: "M1558.444,294s-.955,4.132-1.388,4.5-.4,2.373-.4,2.373,4.258,4.177,4,7.165,2.3-3.415,2.3-3.415,1.705-12.9,3.1-12.778-2.817-2.248-2.817-2.248l.17-1.992c.488-.2-3.473.163-3.734-.922-.145-.6-2.13-.984-2.174-.386-.015.2.177,2.624.177,2.624s-.557,4.166.57,5.065",
      name: "BAHRAIN",
      capital:"Manama",
      area:"765",
      showName: false
  ...
  var methods = {
    ...
    _initCreateStates: function() {
      ...
      for(var state in paths) {
        ...
        this.stateShapes[state] = R.path(paths[state].d).attr(stateAttr);
        this.topShape = this.stateShapes[state];
        // this.addSpan = this.stateName[state];
        this.stateHitAreas[state] = R.path(paths[state].d).attr({fill: "#000",
      "stroke-width": 0, "opacity" : 0.0, 'cursor': 'pointer'});
        this.stateHitAreas[state].node.dataState = state;
        // Write the state name above SVG:
        //console.log(this.stateHitAreas[state].node);
        let node = jQuery(this.stateHitAreas[state].node);
        node.attr("id",paths[state].id);
        let left = node.position().left;
        let top = node.position().top;
        let position = this.stateHitAreas[state].node.getBBox();
        let xPoint = position.x + (position.width/7);
        let yPoint = position.y + (position.height/2.8);

        xPoint *= 0.48956;
        yPoint *= 0.48956;
        //console.log(position.width);
        var span = `<span style='transform: rotate(-50deg,); z-index: 3;position:absolute; color:red;left: ${xPoint}px; top: ${yPoint}px'>${paths[state].name}</span>`; 
        if(paths[state].showName== true){
          $("#map").append(span);
        }
    ...
    _initCreateLabels: function() {
      var R = this.paper; // shorter name for usage here
      var neStates = ['PA', 'LE', 'QA', 'KU', 'DJ', 'BA' , 'JO'];
    ...
    _getState: function(stateName) {
      ...
      var state = paths[stateName];
      var data = {name: state.name, capital: state.capital, area: state.area};
